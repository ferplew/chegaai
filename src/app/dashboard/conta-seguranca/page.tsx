...
const ADMIN_PASSWORD_CHANGE_ENABLED = true; // Set to false to disable password change UI if needed
...
export default function ContaSegurancaPage() {
  ...
  const handleFinanceiroChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    const { name, value } = e.target;
    setFinanceiroData(prev => ({ ...prev, [name]: value }));
    if (name === 'documento') handleDocumentValueChange(value, financeiroData.tipoDocumento);
  };

  const handleDocumentValueChange = (value: string, type: 'cpf' | 'cnpj') => {
  ...
  const handleSaveFinanceiro = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    if (documentError) {
    ...
    // Final check for document validity on submit
            ...
            <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-1">
                        ...
                        <Input id="documento" name="documento" value={financeiroData.documento} onChange={(e) => handleDocumentValueChange(e.target.value, financeiroData.tipoDocumento)} placeholder={financeiroData.tipoDocumento === 'cpf' ? "000.000.000-00" : "00.000.000/0000-00"} className={cn(documentError && "border-destructive")} disabled={isFinanceiroSaving}/>
