...
function OriginalPerfilNegocioForm() {
  ...
  const attemptUnlockSensitiveData = useCallback(() => {
  ...
  }, [adminPasswordInput, toast]);

  const handleChange = useCallback((e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>, section?: keyof PerfilNegocioData) => {
  ...
  const handleNumericChange = useCallback((e: React.ChangeEvent<HTMLInputElement>, field: 'taxaEntrega' | 'tempoMedioEntrega') => {
    const value = e.target.value;
    setFormData(prev => ({ ...prev, [field]: value === '' ? '' : parseFloat(value) }));
  }, []);

  const handleHorarioChange = useCallback((dia: keyof HorariosFuncionamento, campo: keyof HorarioDia, value: string | boolean) => {
    setFormData(prev => ({
      ...prev,
      horarios: {
        ...prev.horarios,
...
const PerfilNegocioForm = React.memo(OriginalPerfilNegocioForm);
export default PerfilNegocioForm;
