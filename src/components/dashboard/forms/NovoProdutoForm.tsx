...
import type { SuggestItemDetailsOutput } from '@/ai/flows/suggest-item-details-flow'; 
import type { GenerateItemImageOutput } from '@/ai/flows/generate-item-image-flow';
...
function OriginalNovoProdutoForm() {
  ...
  const handleSuggestDetails = useCallback(async () => {
    ...
    try {
      const response = await fetch('/api/suggest-item-details', {
        method: 'POST',
      ...
      const result: SuggestItemDetailsOutput = await response.json();
      ...
      toast({ title: "Erro da IA", description: error.message || "Não foi possível gerar sugestões. Tente novamente.", variant: "destructive" });
    } finally {
  ...
  const handleGenerateImage = useCallback(async () => {
    ...
    try {
      ...
      const result: GenerateItemImageOutput = await response.json();
      ...
      toast({ title: "Erro da IA", description: error.message || "Não foi possível gerar a imagem. Tente novamente.", variant: "destructive" });
    } finally {
