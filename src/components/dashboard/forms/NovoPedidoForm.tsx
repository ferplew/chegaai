...
function OriginalNovoPedidoForm() {
  ...
  const handleSubmit = useCallback(async (event: FormEvent<HTMLFormElement>) => {
    ...
    nomeCliente, telefoneCliente, itensPedido, valorTotal, formaPagamento, observacoes,
    enderecoRua, enderecoNumero, enderecoBairro, enderecoCidade, enderecoCep, enderecoComplemento, enderecoReferencia,
    toast, router, resetForm
  ]);
        ...
        <form onSubmit={handleSubmit}>
          ...
          <CardContent className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <Select value={formaPagamento} onValueChange={setFormaPagamento} name="formaPagamento" required disabled={isLoading}>
                ...
                <CardContent className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="space-y-1 md:col-span-2">
                            ...
                            <Input id="enderecoRua" value={enderecoRua} onChange={(e) => setEnderecoRua(e.target.value)} placeholder="Ex: Rua das Palmeiras" disabled={isLoading}/>
                        </div>
                        <div className="space-y-1">
                            <Label htmlFor="enderecoNumero">Número</Label>
                            <Input id="enderecoNumero" value={enderecoNumero} onChange={(e) => setEnderecoNumero(e.target.value)} placeholder="Ex: 123" disabled={isLoading}/>
                        </div>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="space-y-1">
                            <Label htmlFor="enderecoBairro">Bairro</Label>
                            <Input id="enderecoBairro" value={enderecoBairro} onChange={(e) => setEnderecoBairro(e.target.value)} placeholder="Ex: Centro" disabled={isLoading}/>
                        </div>
                        <div className="space-y-1">
                            <Label htmlFor="enderecoCep">CEP</Label>
                            <Input id="enderecoCep" value={enderecoCep} onChange={(e) => setEnderecoCep(e.target.value)} placeholder="Ex: 12345-678" disabled={isLoading}/>
                        </div>
                        <div className="space-y-1">
                            <Label htmlFor="enderecoCidade">Cidade</Label>
                            <Input id="enderecoCidade" value={enderecoCidade} onChange={(e) => setEnderecoCidade(e.target.value)} placeholder="Ex: Cidade Exemplo" disabled={isLoading}/>
                        </div>
                    </div>
                    <div className="space-y-1">
                        <Label htmlFor="enderecoComplemento">Complemento</Label>
                        <Input id="enderecoComplemento" value={enderecoComplemento} onChange={(e) => setEnderecoComplemento(e.target.value)} placeholder="Ex: Apto 101, Bloco B" disabled={isLoading}/>
                    </div>
                    <div className="space-y-1">
                        <Label htmlFor="enderecoReferencia">Ponto de Referência</Label>
                        <Textarea id="enderecoReferencia" value={enderecoReferencia} onChange={(e) => setEnderecoReferencia(e.target.value)} placeholder="Ex: Próximo ao mercado, portão verde" rows={2} disabled={isLoading}/>
                    </div>
                </CardContent>
            </Card>
...
const NovoPedidoForm = React.memo(OriginalNovoPedidoForm);
